 ...
 * module/s used. 
 *
 * Version for iMote2 using its RAM resources
 * 
 ...
 * @modified 15/06/2010: Added PoFlashStorageMemHALI for future compatibility with Flash memory - TomaszT
 */
...
module PoHWMemM {	
    ...
    uses {
      	interface PoRAMMemHALI as PoVRandomMemHALI;
      	interface PoRAMMemHALI as PoNVRandomMemHALI;
      	interface PoRAMMemHALI as PoStorageMemHALI; 
      	interface PoRAMMemHALI as PoFlashStorageMemHALI;     	      	          	      	      
    }
}
implementation {
   ...
   **/
	...
	command void PoMemHALI.flush(PoMemAddr_t addr) {
		//if (IS_FLASH_STORAGE_ADDRESS(addr))
		//		return call PoFlashStorageMemHALI.flush(MASK_PHYSICAL_ADDRESS(addr));
	}
